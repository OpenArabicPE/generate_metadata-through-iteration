...
<xsl:stylesheet exclude-result-prefixes="xs" version="2.0" xmlns:html="http://www.w3.org/1999/xhtml"
    ...
    <xsl:include href="/BachUni/projekte/XML/Functions/DateFunctions%20v1d.xsl"/>

    <xsl:variable name="vgDate" select="current-date()"/>

    <xsl:param name="pgStartDate" select="'1911-05-29'"/>
    <xsl:param name="pgStopDate" select="'1911-08-15'"/>
    <xsl:param name="pgStartImg" select="1"/>
    <xsl:param name="pgStartIssue" select="399"/>
    <xsl:param name="pgVolume" select="9"/>
    <xsl:param name="pgPages" select="8"/>
    ...
    <xsl:param name="pgUrlVar" select="'al-iqbal/mic164_alikbal_1908-1909_'"/>
    ...
    <xsl:template match="*">
        <xsl:result-document
            href="iqbal2Sente {replace($pgStartDate,'-','')}-{replace($pgStopDate,'-','')} {format-date($vgDate,'[Y01][M01][D01]')}.xml"
            method="xml">
            ...
            <xsl:element name="tss:reference">
                ...
                <xsl:element name="tss:characteristics">
                    ...
                    <xsl:element name="tss:characteristic">
                        ...
                        <xsl:value-of select="'al-Iqbāl'"/>
                    </xsl:element>
                    <xsl:element name="tss:characteristic">
                        <xsl:attribute name="name">Short Titel</xsl:attribute>
                        <xsl:value-of select="'Iqbāl'"/>
                    </xsl:element>
                    <!-- volume and issue have been switched due to Sente's filing restrictions -->
                    <xsl:element name="tss:characteristic">
                    ...
                    <xsl:element name="tss:characteristic">
                        ...
                        <xsl:value-of select="'al-Maktaba al-Unsiyya'"/>
                    </xsl:element>
                    <xsl:element name="tss:characteristic">
                    ...
                    <xsl:element name="tss:characteristic">
                        ...
                        <xsl:value-of select="'Mic-Na:164'"/>
                    </xsl:element>
                    <xsl:element name="tss:characteristic">
                        <xsl:attribute name="name">Citation identifier</xsl:attribute>
                        <xsl:value-of select="concat('iqbal ',$pgVolume,'-',./number)"/>
                    </xsl:element>
                    <!-- <xsl:element name="tss:characteristic">
    ...
    <xsl:template name="tIncrementUrl2">
        ...
        <xsl:variable name="vUrlImgBase">
            <xsl:value-of
                select="concat($pgUrlBase,$pgUrlVar)"
            />
        </xsl:variable>
        <xsl:element name="tss:attachmentReference">
            ...
            <xsl:element name="URL">
                <xsl:value-of
                    select="concat($vUrlImgBase, format-number($pNumStart,'0000'),'.jpg')"/>
            </xsl:element>
        </xsl:element>
    ...
    <!-- these templates produce a series of <issue> notes with children for <date>, <number>, <img> -->
    <!-- increment weekly -->
