...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="pgStartDate" select="1887-01-03" as="xs:date"/>
    <xsl:param name="pgStopDate" select="1887-03-01" as="xs:date"/>
    <xsl:param name="pgStartIssue" select="612"/>
    ...
    <xsl:param name="pgUrlVar" select="'1887 612-662'"/>
    ...
    <xsl:template match="*" mode="m4">
        <xsl:result-document href="thamarat2Sente {format-date($pgStartDate,'[Y01][M01][D01]')}-{format-date($pgStopDate,'[Y01][M01][D01]')}{format-date($vgDate,'[Y01][M01][D01]')}.xml" method="xml">
            ...
            <xsl:element name="tss:reference">
                ...
                <xsl:element name="tss:characteristics">
                    ...
                    <xsl:element name="tss:characteristic">
                        ...
                        <xsl:value-of select="concat('tf-oib ',./number)"/>
                    </xsl:element>
                </xsl:element>
    ...
    <xsl:template name="tIncrementUrl">
        ...
        <xsl:variable name="vUrlImgBase">
            <xsl:value-of select="concat($pgUrlBase,$pgUrlVar,'/TF-',year-from-date($pgStartDate),'-')"/>
        </xsl:variable>
    ...
    <xsl:template name="tIncrementWeekly">
        ...
        <xsl:variable name="vDateInc" as="xs:date">
        ...
        <xsl:if test="year-from-date($vDateInc) = year-from-date($pgStartDate)">
            <xsl:call-template name="tIncrementWeekly">
