...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <xsl:apply-templates select="descendant::tei:text/descendant::tei:biblStruct" mode="m_pdf"/>
        <!--<xsl:call-template name="t_generate-fo-file">
            <xsl:with-param name="p_references" select="descendant::tei:biblStruct"/>
        </xsl:call-template>-->
    </xsl:template>
    <xsl:template match="tei:biblStruct" mode="m_pdf">
        <xsl:call-template name="t_generate-fo-file">
            <xsl:with-param name="p_references" select="."/>
            <xsl:with-param name="p_file-name" select="@xml:id"/>
            <xsl:with-param name="p_publication" select="'thamarat'"/>
        </xsl:call-template>
    </xsl:template>
