...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="pPublicationTitle" select="'al-Ittiḥād al-ʿUthmānī'"/>
    <xsl:param name="pShortTitle" select="'Ittiḥād'"/>
    <xsl:param name="pCitId" select="'ittihad'"/>
    <xsl:param name="pPublisher"/>
    <xsl:param name="pPublPlace" select="'Bayrūt'"/>
    <xsl:param name="pgStartDate" select="'1914-11-18'"/>
    <xsl:param name="pgStopDate" select="'1914-12-32'"/>
    <xsl:param name="pgStartImg" select="1"/>
    <xsl:param name="pgStartIssue" select="1868"/>
    ...
    <xsl:param name="pgUrlVar" select="'al-ittihad'"/>
    <xsl:param name="pWeekdayNotPublished" select="'x'"/>
    
    
    <xsl:template match="/">
        <xsl:result-document href="ittihad2Sente {replace($pgStartDate,'-','')}-{replace($pgStopDate,'-','')} {format-date($vgDate,'[Y01][M01][D01]')}.xml" method="xml">
