...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- This stylesheet builds Sente XML for the scans of Sūriye. It does not need any input -->
   
    <!-- v1: production version; currently adapted to Suriye -->
    ...
    <xsl:param name="pgStartDate" select="'1887-01-13'"/>
    <xsl:param name="pgStopDate" select="'1887-03-16'"/>
    <xsl:param name="pgStartImg" select="1"/>
    <xsl:param name="pgStartIssue" select="1093"/>
    <xsl:param name="pgVolume" select="21"/>
    ...
    <xsl:param name="pgUrlBase" select="'/BachUni/projekte/Damascus/sources damascus/Suriye/'"/>
    <xsl:param name="pgUrlVar" select="'a'"/>
    ...
    <xsl:template match="*" mode="m4">
        <xsl:result-document href="suriye2Sente {replace($pgStartDate,'-','')}-{replace($pgStopDate,'-','')} {format-date($vgDate,'[Y01][M01][D01]')}.xml" method="xml">
            ...
            <xsl:element name="tss:reference">
                ...
                <xsl:element name="tss:characteristics">
                    ...
                    <xsl:element name="tss:characteristic">
                        ...
                        <xsl:value-of select="'Sūriyya'"/>
                    </xsl:element>
                    <xsl:element name="tss:characteristic">
                        <xsl:attribute name="name">Short Titel</xsl:attribute>
                        <xsl:value-of select="'Suriye'"/>
                    </xsl:element>
                    <xsl:element name="tss:characteristic">
                    ...
                    <xsl:element name="tss:characteristic">
                        ...
                        <xsl:value-of select="'Arabic / Ottoman'"/>
                    </xsl:element>
                    <xsl:element name="tss:characteristic">
                    ...
                    <xsl:element name="tss:characteristic">
                        ...
                        <xsl:value-of select="'Dimashq'"/>
                    </xsl:element>
                    <xsl:element name="tss:characteristic">
                        <xsl:attribute name="name">publisher</xsl:attribute>
                        <xsl:value-of select="'Maṭbaʿat al-Wilāya'"/>
                    </xsl:element>
                    <xsl:element name="tss:characteristic">
                    ...
                    <xsl:element name="tss:characteristic">
                        ...
                        <xsl:value-of select="concat(format-number(number(tokenize($vDateM,'-')[3]),'0'),' ')"/>
                        <xsl:call-template name="funcDateMonthNameNumber">
                            <xsl:with-param name="pDate" select="$vDateM"/>
                            <xsl:with-param name="pLang" select="'MIjmes'"/>
                        </xsl:call-template>
                        <xsl:value-of select="concat(' ',tokenize($vDateM,'-')[1])"/>
                    ...
                    <xsl:element name="tss:characteristic">
                        ...
                        <xsl:value-of select="concat('suriye-aub ',./number)"/>
                    </xsl:element>
                </xsl:element>
    ...
    <xsl:template name="tIncrementUrl">
        ...
        <xsl:param name="pNumStop"/>
        <xsl:variable name="vUrlImgBase">
            <xsl:value-of select="concat($pgUrlBase,$pgUrlVar,'/',./number,'-')"/>
        </xsl:variable>
        <xsl:variable name="vPage" select="'1a,2a,4a,1t,2t,4t'"/>
        <xsl:for-each select="tokenize($vPage,',')">
            <xsl:variable name="vPage" select="."/>
            <xsl:element name="tss:attachmentReference">
                <xsl:element name="name">
                    <xsl:value-of select="$vPage"/>
                </xsl:element>
                <xsl:element name="URL">
                    <xsl:value-of select="concat($vUrlImgBase,$pNumStart,'_',$vPage,'-300dpi.jpg')"/>
                </xsl:element>
            </xsl:element>
        </xsl:for-each>
        <xsl:if test="number($pNumStart) lt number($pNumStop)">
